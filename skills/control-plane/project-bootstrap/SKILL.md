...
6. Run a dry-run first:
   ...
   ```text
   python3 skills/control-plane/project-bootstrap/scripts/bootstrap_file_project.py MAP.yaml PROJECT_ROOT
   ```

7. Review the actions and then use `--apply` only when the map is correct:

   ```text
   python3 skills/control-plane/project-bootstrap/scripts/bootstrap_file_project.py MAP.yaml PROJECT_ROOT --apply
   ```
