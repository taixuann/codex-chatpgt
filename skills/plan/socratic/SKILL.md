---
name: socratic
description: Surface and resolve material architecture decisions before a plan is accepted. Use for cross-domain design uncertainty; do not use for intent interviews, finished-artifact review, or implementation.
metadata:
  family: plan
  stage: plan
...
Within the `plan` family this is an architecture-preflight leaf. It stops at a
reviewable contract and escalated authority decisions; it does not implement,
commit, or approve the resulting plan.
...
### 7. Architecture-preflight verification

Verify the resulting architecture contract with read-only inspection and
focused checks from the selected domains and packs. Record recommended build
and test commands in the plan packet, but stop before implementation, commit,
or deployment. The downstream build/execution workflow owns those actions.
Report what was verified, what remains `NOT_ASSESSED`, and which authority
decision or approval is still required.
