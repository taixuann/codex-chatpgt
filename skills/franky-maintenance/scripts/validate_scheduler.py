...
def validate(data: object) -> None:
    ...
    if data["mode"] != "scheduled_safe" or data["skill"] != "franky-maintenance" or data["workflow"] != "WF-FRANKY-MAINTENANCE": raise ValueError("invalid Franky scheduled routing")
    ...
    if data.get("model") != "gpt-5.4-mini" or data.get("reasoning_effort") != "medium": raise ValueError("low-cost model defaults are required")
