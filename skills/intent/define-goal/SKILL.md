---
name: define-goal
description: Define a concrete, measurable goal from a chosen user or GitHub Issue direction. Use for goal creation or refinement only; do not create plans, snapshots, or execution artifacts.
metadata:
  family: intent
  stage: intent
...
## Pre-Execution Check & Discovery (Step 0)

Before defining a new goal, always execute the self-contained query engine:

```bash
python3 skills/intent/define-goal/scripts/query_existing_goals.py "<user_intent_keywords>" --json
```

1. **Deduplication**: If an active or past goal already matches the intent, prompt to upgrade the existing goal rather than creating a duplicate.
2. **Semantic Goal Versioning (`vA.B.C`)**:
   - **`A` (Big Leaf)**: Epoch baseline rewrite / core objective change.
   - **`B` (Major)**: Major plan phase / feature milestone addition.
   - **`C` (Minor)**: Task refinement, patch, or detail strengthening.
3. **Multi-Goal Hierarchy**: If the intent is large and multifaceted, split into a parent goal (`GOAL-101`) and child sub-goals (`GOAL-101.1`).
