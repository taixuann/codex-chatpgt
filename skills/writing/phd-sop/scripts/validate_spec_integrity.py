...
def main() -> int:
    ...
    for group in cmap.get("groups", []):
        for member in group.get("members", []):
            if member not in all_ids:
                errors.append(f"construct group {group.get('id')} references unknown id {member}")

    # Eval/anchor references must remain valid.
    coverage = set()
    ...
    if missing_coverage:
        errors.append(f"canonical IDs without fixture/anchor coverage: {missing_coverage}")

    # Readiness references must be canonical and ordinal.
    for row in rubric.get("readiness", {}).get("core_minimums", []):
        rid = row.get("id")
        if rid not in criterion_ids:
            errors.append(f"readiness minimum must target ordinal criterion: {rid}")
    ...
    required_sources = {
        ...
        "stanford_sop_guidance","duke_sop_guidance","cornell_asop_guidance"
    }
    ...
    if errors:
        ...
        return 1
    print(f"PASS: referential closure for {len(all_ids)} canonical IDs; coverage complete")
    return 0
