...
**Validation:** Run `scripts/validate_rubric.py`, `scripts/validate_references.py`, `scripts/validate_spec_integrity.py`, and the calibration-harness self-test in `scripts/evaluate_calibration.py`. The self-test validates the measurement harness only; behavioral writing quality remains `NOT_ASSESSED` until real calibration runs exist. A candidate is not ready while any validator fails or a material hard gate remains unresolved.
