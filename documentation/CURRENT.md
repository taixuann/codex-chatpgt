...
- **#35** — current skill dispositions and all seventeen retired Franky workflow YAMLs are recorded in `PLAN-ARW-SYSTEM-SKILLS-V2-20260810-001.md`. Disposable OpenCode probes now directly confirm configured-path discovery, project-local shadowing for a colliding skill ID, and a 9-entry source-root swap when external-skill scan flags are toggled; the reconciliation remains conditional until host-observable co-loaded activation and cross-runtime behavior exist. No new target skill zoo or workflow engine was created.
