...
def validate_cases(path: Path) -> None:
    ...
    for case_id, case in by_id.items():
        ...
        if case_id in {"implementation_routes_prometheus", "missing_context_routes_argus", "control_plane_routes_franky"} and (result.get("spawned") is not False or expect.get("parent_routes_only") is not True): raise ValueError(f"case {case_id}: parent-only routing assertion missing")
        if case_id == "material_mutation_requires_rereview" and expect.get("fresh_review_required") is not True: raise ValueError("mutation case must require fresh review")
